...
const OPERATIONAL_LEAK_PATTERNS = [
  ...
  /^我(?:先|会|来|去|再|马上)?把.*(?:这段|这条|这个|推进|学习|练习).*(?:记成|记为|记进|记到|写进|放进).*(?:记录|日记|时间线|时间轴|timeline)/i,
  /^我(?:先|会|来|去|再|马上)?把.*(?:这条|那条|这个|那个).*(?:待办|提醒|未收尾|没收尾).*(?:收掉|关掉|关闭|取消|作废|完成|移除|不再提醒|挂着)/i,
  /^我(?:先|会|来|去|再|马上)?把(?:这条|那条|这个|那个).*(?:收掉|关掉|关闭|取消|作废|完成).*(?:跳出来|提醒|挂着|烦你)/i,
  /^我(?:先|会|来|去|再|马上|在)?找.*(?:提醒|待办|入口|状态).*(?:入口|作废|更新|取消|关闭|收掉|烦你|不让它)/i,
  /^我(?:就|会|先|再|马上)?把.*(?:状态|提醒|待办).*(?:更新|改成|标成|设成|收掉|关闭|作废).*(?:完成|已完成|作废|取消|关闭|收掉|不让它|烦你)/i,
  /^刚才.*(?:时间线|时间轴|timeline).*(?:写入|记录|分类|category|taxonomy|节点|node).*(?:卡住|卡住了|对上|落到|落稳|分类|节点|node|taxonomy)/i,
  /^(?:第一次|首次|这次|刚才).*(?:写入|记录).*(?:少了|缺少|没有|没带).*(?:分类|category|taxonomy|节点|node)/i,
  /^我(?:找到|已经找到|看到了?).*(?:timeline|时间线|时间轴|工程).*(?:taxonomy|分类|category|节点|node)/i,
  /^我(?:已经)?对上了?.*(?:现成|已有).*(?:节点|node|category|taxonomy|分类|学习练习|study\.[\w.-]+)/i,
  /(?:taxonomy|categoryId|subcategoryId|eventNodeId|study\.[\w.-]+|timeline 工程|时间线工程).*(?:节点|node|分类|category|写入|记录|落稳|落到|定义|新造)/i,
  /(?:现成|已有).*(?:study\.[\w.-]+|节点|node|taxonomy|category|分类|学习练习).*(?:不需要|不用|不).*(?:新造|新增|创建).*(?:分类|节点|category|node)?/i,
