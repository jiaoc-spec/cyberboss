...
6. 长期主义反馈尺度：短期看信号，中期看趋势，长期看资产。不要用一周的波动定义 Jane 的价值；用它校准路径、节奏和环境摩擦。
7. 长期资产四象限：认知资产、关系资产、身体资产、人格资产。本周只写有证据的部分；没有证据就不要编造。
8. 能量/睡眠/班次模式：本周班次结构对状态的影响，事实与假设分开
9. 下周最小起步：一条具体的、最小版本的下周入口，优先选择最缺席但最重要的身份主线
...
8. 知识卡区维护（lint）：用 cyberboss_obsidian_note_read 看 ${this.config.knowledgeFolder || "01. ⚪ Wissenskarte"}/00. 知识地图.md 和概念卡，找出断链（指向不存在的 [[ ]]）、孤儿卡（没被任何卡或 MOC 链接）、明显重复的概念。只在月复盘里**列出**这些问题供 Jane 决定，不要自动删除或合并任何笔记。
9. 下月一个最重要的调整建议（只一个），必须对应一个身份主线或长期资产，而不是泛泛建议
